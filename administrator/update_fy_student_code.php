...
if(empty($s_name) || empty($s_gender)|| empty($s_mob) || empty($s_mail_id) || empty($s_gender) || empty($s_fn) ||empty($s_mn) ||empty($s_dob) ||empty($s_address) || empty($seatno) || empty($institutename) ||empty($instituteno) ||empty($centerno) ||empty($examyear))
	...
	elseif(!preg_match('/^[_a-zA-Z0-9-]+(\.[_a-zA-Z0-9-]+)_a-z0-9-]+(\.[_a-z0-9-]+)_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,4})$/',$s_mail_id)){
		$status = "Enter valid Email Id.";
        header("location: edit_fy_student.php?status=".$status);
	}
	elseif(!preg_match($reggender,$s_gender)){
		$status = "Select Gender";
        header("location: edit_fy_student.php?status=".$status);
	}
	else{
$sql = "UPDATE student SET s_name='$s_name',s_fn='$s_fn',s_mn='$s_mn',s_gender='$s_gender',s_dob='$s_dob', s_mob='$s_mob', s_mail_id='$s_mail_id',s_address='$s_address',seatno='$seatno',institutename='$institutename',instituteno='$instituteno',centerno='$centerno',examyear='$examyear' WHERE s_id=$s_id";
