...
if(empty($s_name) || empty($s_gender)|| empty($s_mob) || empty($s_mail_id) || empty($s_gender) || empty($s_fn) ||empty($s_mn) ||empty($s_dob) ||empty($s_address) || empty($seatno) || empty($institutename) ||empty($instituteno) ||empty($centerno) ||empty($examyear))
	{
        ...
        header("location: add_sy_student.php?status=".$status);
	}	
	...
	elseif(!preg_match('/^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,4})$/',$s_mail_id)){
		...
		mysqli_query($con,"INSERT INTO sy1_student (s_id,s_name,s_fn,s_mn,s_gender,s_dob,s_mob,s_mail_id,s_address,seatno,institutename,instituteno,centerno,examyear) VALUES 
